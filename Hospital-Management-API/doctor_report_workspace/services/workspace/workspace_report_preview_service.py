"""WorkspaceReportPreviewService — authorize → resolve artifact → audit → access URL."""
...
from doctor_report_workspace.dto import PreviewResponseDTO
...
class WorkspaceReportPreviewValidationError(ValueError):
    """Invalid preview request parameters."""


@dataclass(frozen=True)
class PreviewResult:
    """Internal orchestration result for the preview view."""
    ...
    def to_unsupported_dto(self) -> PreviewResponseDTO:
...
def _local_file_streamable(artifact: Any) -> bool:
    """True when local storage mode is active and the artifact file exists."""
...
class WorkspaceReportPreviewService:
    """Orchestrate preview access without ORM or storage details in the view."""
    ...
    def get_preview(
        ...
        *,
        ...
        aggregate = self._repository.get_preview_artifact(scope, report_uuid)
        ...
        try:
            ...
            raise WorkspaceReportPreviewValidationError(str(exc)) from exc

        if resolved is None:
            duration_ms = int((time.perf_counter() - started) * 1000)
            logger.info(
                "Workspace report preview not supported",
            ...
            return PreviewResult(supported=False)
        ...
        try:
            ...
            # Local/dev: S3 unset — stream authenticated file bytes (never public media URL).
        ...
        duration_ms = int((time.perf_counter() - started) * 1000)
        logger.info(
            "Workspace report preview completed",
            module=LogModule.REPORTS,
            action="doctor_report_workspace.preview",
            metadata={
                "report_uuid": str(report_uuid),
    ...
    def expires_at_iso(expires_in: int) -> str:
        return (timezone.now() + timedelta(seconds=expires_in)).isoformat()

    @staticmethod
    def _require_uuid(value: Any) -> str:
        if value is None or value == "":
            raise WorkspaceReportPreviewValidationError("report_id is required.")
        try:
            return str(UUID(str(value)))
        except (ValueError, TypeError, AttributeError) as exc:
            raise WorkspaceReportPreviewValidationError(
