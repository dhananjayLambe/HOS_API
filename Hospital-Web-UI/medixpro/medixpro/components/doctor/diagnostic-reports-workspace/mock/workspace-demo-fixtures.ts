...
function pdfPreviewHtml(title: string, patientName: string): string {
  ...
  <h1>${title}</h1>
  <p class="muted">Patient: ${patientName} · Demo diagnostic report</p>
  ...
  <p class="foot">DoctorProCare · Demo artifact — not for clinical use</p>
  </body></html>`;
}
...
function patient(id: string): WorkspacePatient {
  ...
  if (!found) throw new Error(`Unknown demo patient ${id}`);
  return found;
}
...
export function createDemoReports(): WorkspaceReport[] {
