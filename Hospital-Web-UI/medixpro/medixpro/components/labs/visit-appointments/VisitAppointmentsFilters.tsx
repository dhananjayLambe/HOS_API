...
import {
  VISIT_APPOINTMENTS_DATE_OPTIONS,
  VISIT_APPOINTMENTS_TAB_OPTIONS,
...
type Props = {
  ...
  filters: VisitAppointmentsFilterState;
  onFiltersChange: (next: VisitAppointmentsFilterState) => void;
  disabled?: boolean;
};

export function VisitAppointmentsFilters({
  searchInput,
  onSearchChange,
  filters,
  onFiltersChange,
  ...
  return (
    <div className="space-y-3">
      <LabFilterBar className="flex-wrap gap-2">
        {VISIT_APPOINTMENTS_TAB_OPTIONS.map((tab) => (
          <Button
            key={tab.id}
            type="button"
            size="sm"
      ...
      <LabFilterBar className="flex-col items-stretch gap-4 sm:flex-row sm:items-end">
        <div className="w-full min-w-[200px] flex-1 sm:max-w-xs">
          ...
          <div className="relative mt-1">
            ...
            <Input
              className="h-9 pl-9"
              placeholder="Patient, phone, appointment ID"
              value={searchInput}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {VISIT_APPOINTMENTS_DATE_OPTIONS.map((opt) => (
            <Button
              key={opt.id}
              type="button"
              size="sm"
