...
import { useClinicalLabHistorySummary } from "./clinical-lab-history";
...
export function PatientSummaryHeader({
  ...
  const labSummary = useClinicalLabHistorySummary(patient.id);
  const pendingLabs = labSummary.data?.pending ?? quick_stats.pending_labs;

  const badges: Array<{ key: string; label: string; show: boolean }> = [
    {
      key: "consultation_active",
      label: "Consultation Active",
      show: patient.open_encounter_state === "consultation_active",
    },
    {
      key: "in_queue",
      label: "In Queue",
      show: patient.open_encounter_state === "in_queue",
    },
    { key: "follow_up_due", label: "Follow-up Due", show: patient.is_follow_up_due },
    {
      key: "recent_visit",
      label: "Recent Visit",
      show: quick_stats.last_visit_label === "Today",
    },
  ];
  const topBadges = badges.filter((b) => b.show).slice(0, 2);
  const consultLabel =
    patient.has_open_encounter || patient.has_unfinished_consultation
      ? "Continue Consultation"
      : "Start Consultation";
  const primaryState =
    patient.open_encounter_state === "consultation_active"
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
        <div className="flex min-w-0 items-start gap-4">
          ...
          <div className="min-w-0 space-y-3">
            <p className="truncate text-[28px] font-semibold leading-none tracking-tight text-slate-900">
              {patient.full_name}
            </p>
        ...
        <div className="flex flex-col items-stretch gap-2 sm:flex-row sm:flex-wrap sm:items-center lg:justify-start">
          <Button
            className="min-h-[44px] bg-primary px-4 text-sm text-primary-foreground hover:bg-primary/90"
          ...
          <Button
            variant="ghost"
            className="min-h-[44px] px-3 text-sm text-slate-600 hover:bg-slate-100 hover:text-slate-900"
