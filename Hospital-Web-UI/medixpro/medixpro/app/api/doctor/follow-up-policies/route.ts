...
export async function GET(request: NextRequest) {
  try {
    console.log("[Next.js API] GET /api/doctor/follow-up-policies - Request received")
    ...
    if (!response.ok) {
      ...
      console.error("[Next.js API] Django error:", data)
      return NextResponse.json(
        {
    ...
    console.error("Follow-up policies fetch error:", error)
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    console.log("[Next.js API] POST /api/doctor/follow-up-policies - Request received")
    const token = request.headers.get("Authorization")
    ...
    console.error("Follow-up policy create error:", error)
    return NextResponse.json(
      { error: error.message || "Internal server error" },
      { status: 500 }
    )
