...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${DJANGO_API_URL}tasks/${queryString ? `?${queryString}` : ""}`
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          error: data.message || data.detail || "Failed to fetch tasks",
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[Next.js API] POST /api/tasks - Request body:", JSON.stringify(body, null, 2))

    const response = await fetch(`${DJANGO_API_URL}tasks/`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(authHeader && { Authorization: authHeader }),
    ...
    if (!response.ok) {
      ...
      let errorMessage = data.message || data.detail || "Failed to create task"
    ...
    console.error("[Next.js API] Task creation error:", error)
    return NextResponse.json(
      {
        error: error.message || "Internal server error",
        details: error.stack,
