...
export function computeCompletionKpis(orders: OrderLifecycleViewModel[]): CompletionKpis {
  ...
  return { pendingUploads, readyToSend, urgentDelays, deliveryFailures };
}

export function filterOrdersByChip(
  orders: OrderLifecycleViewModel[],
  filter: CompletionFilterKey,
): OrderLifecycleViewModel[] {
  if (filter === "all") return orders.filter((o) => !o.isFullyComplete);
  ...
  if (filter === "failed") {
    return orders.filter(
      (o) =>
        !o.isFullyComplete &&
        (o.deliveryFailure != null || o.reports.some(isFailedReport)),
    );
  }
  return orders;
}
...
export function orderPriorityScore(o: OrderLifecycleViewModel): number {
  ...
  if (o.tatState === "breached") return 1;
  if (o.urgency === "STAT" && o.hasPendingUpload) return 2;
  if (o.reports.some(isUpdatedReportPendingSend)) return 3;
  if (o.attentionReasons.includes("stuck_partial")) return 4;
  if (o.hasPendingUpload) return 5;
  if (o.reports.some(isReadyToSend)) return 6;
  if (o.tatState === "near_breach") return 7;
  if (o.urgency === "STAT" || o.urgency === "URGENT") return 8;
  return 9;
}
